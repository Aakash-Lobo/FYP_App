...
try {
    ...
    $sqlCategory = "SELECT COUNT(*) AS 'Total Rooms' FROM rooms";
    $resultCategory = mysqli_query($con, $sqlCategory);
    $categoryCount = mysqli_fetch_assoc($resultCategory);
    $categoryCount = intval($categoryCount['Total Rooms']); // Convert to integer

    $sqlCompany = "SELECT COUNT(*) AS 'Total Regs' FROM roomregistration";
    $resultCompany = mysqli_query($con, $sqlCompany);
    $companyCount = mysqli_fetch_assoc($resultCompany);
    $companyCount = intval($companyCount['Total Regs']); // Convert to integer
    ...
    // Prepare data
    $statistics = [
        'Total Rooms' => $categoryCount,
        'Total Regs' => $companyCount,
