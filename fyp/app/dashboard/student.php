...
try {
    ...
    $sqlCategory = "SELECT COUNT(*) AS 'Total Students' FROM student_info";
    $resultCategory = mysqli_query($con, $sqlCategory);
    $categoryCount = mysqli_fetch_assoc($resultCategory);
    $categoryCount = intval($categoryCount['Total Students']); // Convert to integer

    $sqlCompany = "SELECT COUNT(*) AS 'Total Courses' FROM student_courses";
