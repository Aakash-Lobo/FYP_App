...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($pizzaId != null && $newPizzaName != null && $newPizzaDesc != null) {
        ...
        $updateSql = "UPDATE `pizza` SET pizzaName = '$newPizzaName', pizzaDesc = '$newPizzaDesc' WHERE pizzaId = $pizzaId";

        if (mysqli_query($conn, $updateSql)) {
            $response = array('status' => 'success', 'message' => 'Pizza updated successfully');
        } else {
            $response = array('status' => 'error', 'message' => 'Error updating pizza: ' . mysqli_error($conn));
        }
    } else {
